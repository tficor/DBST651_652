...
CREATE OR REPLACE TYPE XDB$RAW_LIST_T IS VARRAY ( 1000 ) OF UNKNOWN ;
/

CREATE OR REPLACE TYPE ACTION_V IS VARRAY ( 4 ) OF UNKNOWN ;
/
...
AS
  ...
  (
    ...
    RETURN VARCHAR2 ) NOT FINAL NOT INSTANTIABLE ;
  /
...
CREATE OR REPLACE TYPE COMPOSITE_CATEGORY_TYP UNDER CATEGORY_TYP ( SUBCATEGORY_REF_LIST SUBCATEGORY_REF_LIST_TYP , MEMBER FUNCTION CATEGORY_DESCRIBE
  RETURN VARCHAR2 ) NOT FINAL ;
/

CREATE OR REPLACE TYPE CATALOG_TYP UNDER COMPOSITE_CATEGORY_TYP ( MEMBER FUNCTION GETCATALOGNAME
  RETURN VARCHAR2 , MEMBER FUNCTION CATEGORY_DESCRIBE
...
AS
  ...
  (
    ...
    COUNTRY_ID     CHAR (2) ) FINAL ;
  /

CREATE OR REPLACE TYPE PHONE_LIST_TYP IS VARRAY ( 5 ) OF UNKNOWN ;
/
...
AS
  ...
  (
    ...
    CUST_ORDERS ORDER_LIST_TYP ) NOT FINAL ;
  /

CREATE OR REPLACE TYPE CORPORATE_CUSTOMER_TYP UNDER CUSTOMER_TYP ( ACCOUNT_MGR_ID NUMBER (6) ) FINAL ;
/
...
AS
  ...
  (
    ...
    QUANTITY_ON_HAND NUMBER (8) ) FINAL ;
  /
...
CREATE OR REPLACE TYPE LEAF_CATEGORY_TYP UNDER CATEGORY_TYP ( PRODUCT_REF_LIST PRODUCT_REF_LIST_TYP , MEMBER FUNCTION CATEGORY_DESCRIBE
  RETURN VARCHAR2 ) FINAL ;
/

CREATE OR REPLACE TYPE LINEITEM_V IS VARRAY ( 2147483647 ) OF UNKNOWN ;
/
...
AS
  ...
  (
    ...
    QUANTITY    NUMBER (8,4) ,
    UNITPRICE   NUMBER (12,2) ) NOT FINAL ;
  /
...
AS
  ...
  (
    ...
    QUANTITY     NUMBER (8) ,
    PRODUCT_REF REF PRODUCT_INFORMATION_TYP ) FINAL ;
  /
...
AS
  ...
  (
    ...
    ORDER_ITEM_LIST ORDER_ITEM_LIST_TYP ) FINAL ;
  /
...
AS
  ...
  (
    ...
    INVENTORY_LIST INVENTORY_LIST_TYP ) FINAL ;
  /
...
AS
  ...
  (
    ...
    DATE_REJECTED   DATE ,
    REASON_REJECTED VARCHAR2 (2048) ) NOT FINAL ;
  /
...
AS
  ...
  (
    ...
    SHIP_TO_PHONE   VARCHAR2 (24) ) NOT FINAL ;
  /
...
AS
  ...
  (
    ...
    LINEITEMS LINEITEMS_T ) NOT FINAL ;
  /

CREATE OR REPLACE TYPE SDO_ELEM_INFO_ARRAY IS VARRAY ( 1048576 ) OF UNKNOWN ;
/

CREATE OR REPLACE TYPE SDO_ORDINATE_ARRAY IS VARRAY ( 1048576 ) OF UNKNOWN ;
/
...
COMMENT ON COLUMN OE.CUSTOMERS.CUSTOMER_ID
IS
  'Primary key column.' ;
  COMMENT ON COLUMN OE.CUSTOMERS.CUST_FIRST_NAME
IS
  'NOT NULL constraint.' ;
  COMMENT ON COLUMN OE.CUSTOMERS.CUST_LAST_NAME
IS
  'NOT NULL constraint.' ;
  COMMENT ON COLUMN OE.CUSTOMERS.CUST_ADDRESS
IS
  'Object column of type address_typ.' ;
  COMMENT ON COLUMN OE.CUSTOMERS.PHONE_NUMBERS
IS
  'Varray column of type phone_list_typ' ;
  COMMENT ON COLUMN OE.CUSTOMERS.CREDIT_LIMIT
IS
  'Check constraint.' ;
  COMMENT ON COLUMN OE.CUSTOMERS.ACCOUNT_MGR_ID
IS
  'References hr.employees.employee_id.' ;
  COMMENT ON COLUMN OE.CUSTOMERS.CUST_GEO_LOCATION
IS
  'SDO (spatial) column.' ;
...
COMMENT ON COLUMN OE.INVENTORIES.PRODUCT_ID
IS
  'Part of concatenated primary key, references product_information.product_id.' ;
  COMMENT ON COLUMN OE.INVENTORIES.WAREHOUSE_ID
IS
...
COMMENT ON COLUMN OE.ORDERS.ORDER_ID
IS
  'PRIMARY KEY column.' ;
  COMMENT ON COLUMN OE.ORDERS.ORDER_DATE
IS
  'TIMESTAMP WITH LOCAL TIME ZONE column, NOT NULL constraint.' ;
  COMMENT ON COLUMN OE.ORDERS.ORDER_MODE
IS
  'CHECK constraint.' ;
  COMMENT ON COLUMN OE.ORDERS.ORDER_STATUS
IS
...
10: Shipped - paid' ;
  COMMENT ON COLUMN OE.ORDERS.ORDER_TOTAL
IS
  'CHECK constraint.' ;
  COMMENT ON COLUMN OE.ORDERS.SALES_REP_ID
IS
  'References hr.employees.employee_id.' ;
  COMMENT ON COLUMN OE.ORDERS.PROMOTION_ID
IS
...
COMMENT ON COLUMN OE.ORDER_ITEMS.ORDER_ID
IS
  'Part of concatenated primary key, references orders.order_id.' ;
  COMMENT ON COLUMN OE.ORDER_ITEMS.LINE_ITEM_ID
IS
  'Part of concatenated primary key.' ;
  COMMENT ON COLUMN OE.ORDER_ITEMS.PRODUCT_ID
IS
...
COMMENT ON COLUMN OE.PRODUCT_DESCRIPTIONS.PRODUCT_ID
IS
  'Primary key column.' ;
  COMMENT ON COLUMN OE.PRODUCT_DESCRIPTIONS.LANGUAGE_ID
IS
  'Primary key column.' ;
CREATE UNIQUE INDEX OE.PRD_DESC_PK ON OE.PRODUCT_DESCRIPTIONS
  (
    PRODUCT_ID ASC , LANGUAGE_ID ASC
  )
  ;
...
COMMENT ON COLUMN OE.PRODUCT_INFORMATION.PRODUCT_ID
IS
  'Primary key column.' ;
  COMMENT ON COLUMN OE.PRODUCT_INFORMATION.PRODUCT_DESCRIPTION
IS
...
to accss.' ;
  COMMENT ON COLUMN OE.PRODUCT_INFORMATION.CATEGORY_ID
IS
  'Low cardinality column, can be used for bitmap index.
Schema SH uses it as foreign key' ;
  COMMENT ON COLUMN OE.PRODUCT_INFORMATION.WEIGHT_CLASS
IS
  'Low cardinality column, can be used for bitmap index.' ;
  COMMENT ON COLUMN OE.PRODUCT_INFORMATION.WARRANTY_PERIOD
IS
  'INTERVAL YEAER TO MONTH column, low cardinality, can be used for bitmap
index.' ;
  COMMENT ON COLUMN OE.PRODUCT_INFORMATION.SUPPLIER_ID
IS
  'Offers possibility of extensions outside Common Schema.' ;
  COMMENT ON COLUMN OE.PRODUCT_INFORMATION.PRODUCT_STATUS
IS
...
COMMENT ON COLUMN OE.WAREHOUSES.WAREHOUSE_ID
IS
  'Primary key column.' ;
  COMMENT ON COLUMN OE.WAREHOUSES.WH_GEO_LOCATION
IS
...
ALTER TABLE OE.ORDERS ADD CONSTRAINT ORDERS_CUSTOMER_ID_FK FOREIGN KEY ( CUSTOMER_ID ) REFERENCES OE.CUSTOMERS ( CUSTOMER_ID ) ON
DELETE SET NULL ;
...
ALTER TABLE OE.PRODUCT_DESCRIPTIONS ADD CONSTRAINT PD_PRODUCT_ID_FK FOREIGN KEY ( PRODUCT_ID ) REFERENCES OE.PRODUCT_INFORMATION ( PRODUCT_ID ) ;
